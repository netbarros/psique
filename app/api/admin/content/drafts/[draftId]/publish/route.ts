...
function revalidatePublishedPage(pageKey: string) {
  const pathMap: Record<string, string[]> = {
    landing: ["/"],
    pricing: ["/pricing"],
    checkout_secure: ["/checkout/secure"],
    booking: ["/booking/[slug]"],
    booking_success: ["/booking/[slug]/sucesso"],
  };
...
export async function POST(request: Request, { params }: RouteContext) {
  const route = "/api/admin/content/drafts/[draftId]/publish";
  const auth = await requireMasterAdminContext();
  if (!auth.context) {
    logger.warn("[Admin/Content] Unauthorized publish attempt", { route });
  ...
  if (targetRevisionError) {
    logger.error("[Admin/Content] Failed loading revision for publish", {
      route,
  ...
  if (archivePreviousError) {
    logger.error("[Admin/Content] Failed archiving previous published revision", {
      route,
      userId: auth.context.user.id,
      draftId,
  ...
  if (publishError || !publishedRevision) {
    logger.error("[Admin/Content] Failed publishing revision", {
      route,
      userId: auth.context.user.id,
      draftId,
  ...
  const document = pickContentDocument((publishedRevision as { content_documents?: unknown }).content_documents);
  ...
  if (document.page_key) {
    revalidatePublishedPage(document.page_key);
  }

  logger.info("[Admin/Content] Revision published", {
    route,
    userId: auth.context.user.id,
    draftId,
    documentId: publishedRevision.document_id,
